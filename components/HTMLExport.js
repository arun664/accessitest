...
const HTMLExport = ({ selectedItems }) => {
  ...
  const generateChartImage = () => {
    return new Promise((resolve) => {
      ...
      const totalIssues = dataCounts.violations + dataCounts.incomplete + dataCounts.passes || 1;
      ...
      const renderTable = (title, data) => {
        ...
        return `
          <div class="accordion-item">
            <div class="accordion-header" onclick="document.getElementById('body${index}-${title}').classList.toggle('show');">
              ${title}
              <span class="toggle-icon">+</span>
            </div>
            <div class="accordion-body" id="body${index}-${title}">
              <table>
                <tr>
                  <th>HTML Element</th>
                ...
                ${data.flatMap(issue =>
                  issue.nodes.map(node => `
                    <tr>
                      ...
                      <td>${node.any && node.any.length > 0 ? node.any[0].message : 'No issues'}</td>
                      <td>${node.any && node.any.length > 0 ? JSON.stringify(node.any[0].data || {}) : 'N/A'}</td>
                    </tr>
                  `).join('')
                ).join('')}
              </table>
